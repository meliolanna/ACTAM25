...
class SoundScheduler {
  constructor(durationSequence) {
    ...
    // tempi assoluti di ATTACCO delle note (N click, non N+1)
  ...
  scheduleClick(time) {
    ...
    osc.frequency.setValueAtTime(1500, time); // metronomo è 1000Hz
    osc.connect(gain);
...
class PatternRepeatMiniGame {
  ...
  generatePattern(bpm) {
    ...
    this.soundScheduler = new SoundScheduler(durationsSec);
    this.patternPlayed = false;
   ...
   * da targetBeatIndex.
   */
  onInput(gameModel, _deltaMs, _targetBeatIndex, nowMs) {
    const bpm = gameModel.bpm;
    const beatDurationMs = 60000 / bpm;
    const measureDurationMs = 4 * beatDurationMs;

    // Se per qualche motivo non è ancora partita la battuta di input
...
model.addMiniGame(new PatternRepeatMiniGame());
